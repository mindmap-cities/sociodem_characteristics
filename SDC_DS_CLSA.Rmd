...
opal.file_download(erasmus_opal,"/home/clsa/171013_McMaster_PRaina _CoP4_Baseline.csv","/home/rwissa/sociodem_characteristics/cop4_baseline.csv")
...
opal.file_download(erasmus_opal,"/home/clsa/171013_McMaster_PRaina _CoP_FU1.csv","/home/rwissa/sociodem_characteristics/cop4_fu1.csv")

```
## Assign participant id as first column of your harmonized dataframes
```{r, echo=FALSE}
sdc_CLSA_0 <- data.frame(id = cop4_baseline$entity_id )
sdc_CLSA_1 <- data.frame(id = cop4_fu1$entity_id )
```
# Table of Contents
...
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_0$sdc_age_0<- cop4_baseline$AGE_NMBR_COM
```  
### **Variable label**: Age of participant   
...
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_1$sdc_age_1<- cop4_fu1$AGE_NMBR_COF1
```  
### **Variable label**: Age of participant   
...
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_0$sdc_gender_0<- recode(cop4_baseline$SEX_ASK_COM, 'F'=1L, 'M'=0L, .default = NA_integer_)
```  
### **Variable label**: Gender of participant   
...
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_1$sdc_gender_1<- recode(cop4_fu1$SDC_BTHSEX_COF1, '2'=1L, '1'=0L, .default = NA_integer_)
```  
### **Variable label**: Gender of participant   
...
**Harmonization status**:  complete  
**Harmonization comment**:   redo to include skip pattern  
**R script**:  
```{r, echo=TRUE}
#sdc_CLSA_0$sdc_employ_0<- recode(cop4_baseline$LBF_CURR_COM, '1'=1L,'2'=0L, .default=NA_integer_)
...
**R script**:  
```{r, echo=TRUE}
#sdc_CLSA_1$sdc_employ_1<- recode(cop4_fu1$LBF_CURR_COF1, '1'=1L,'2'=0L, .default=NA_integer_)
sdc_CLSA_1$sdc_employ_1 <- ifelse((sdc_CLSA_0$sdc_retire_0==1 & cop4_fu1$RET_RTRN_COF1==2) | cop4_fu1$RET_RTRD_COF1==1,0L,recode(cop4_fu1$LBF_CURR_COF1, '1'=1L,'2'=0L, .default=NA_integer_))
```  
### **Variable label**: Employment status   
...
**Harmonization status**:  complete  
**Harmonization comment**:   Study-specific variable is categorized as: Completely retired | Partly retired | Not retired.  
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_0$sdc_retire_0<- recode(cop4_baseline$RET_RTRD_COM,'1'=1L,'2'=1L,'3'=0L, .default= NA_integer_)
```  
### **Variable label**: Retirement status   
...
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_0$sdc_retire_age_0<- ifelse(cop4_baseline$RET_AGE_NB_COM<98,cop4_baseline$RET_AGE_NB_COM,NA)
```  
### **Variable label**: Retirement age    
...
**Harmonization status**:  impossible  
**Harmonization comment**: Study-specific variable is total household income before taxes and deductions.  
**R script**:  
```{r, echo=TRUE}
...
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_0$sdc_civstat_0<- recode(cop4_baseline$SDC_MRTL_COM, '1'=0L,'2'=1L,'3'=3L,'4'=2L,'5'=2L, .default=NA_integer_)
```  
### **Variable label**: Civil status    
...
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_0$sdc_livstat_0<- recode(cop4_baseline$SDC_MRTL_COM,'1'=0L, '2'=1L, '3'=0L,'4'=0L,'5'=0L,.default=NA_integer_)
```  
### **Variable label**: Married or living with partner  
...
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_0$sdc_num_pplhh_0 <- cop4_baseline$SN_LIVH_NB_COM+1
```  
### **Variable label**: Number of people in household  
...
**R script**:  
```{r, echo=TRUE}
sdc_CLSA_0$sdc_num_childhh_0<- cop4_baseline$SN_LIVHKD_COM
```  
### **Variable label**: Number of children in household  
