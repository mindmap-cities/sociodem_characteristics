...
**R script**:  
```{r, echo=TRUE}
sdc_HAPIEE_LT_0$sdc_employ_0<- recode(as.integer(HAPIEE_W2$paidwork),'1'=1L,'2'=0L,.default = NA_integer_)
...
sdc_HAPIEE_LT_0$sdc_retire_0<- recode(as.integer(HAPIEE_W2$retired),'1'=1L,'2'=0L,.default=NA_integer_)
...
**Correctin comment **: Incompatible: in the study-specific variable "Highest completed level of education", it is impossible to distinguish between upper and lower secondary for participants who completed secondary education.  
**R script**:  
```{r, echo=TRUE}
...
**R script**:  
```{r, echo=TRUE}
sdc_HAPIEE_LT_0$sdc_num_pplhh_0<-as.integer(HAPIEE_W2$numhouse)
